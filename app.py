...
:root {
  ...
  --red: #a01a1a; --cream: #e0e0d1; --muted: #444444; --dim: #2a2a2a;
}
...
.stSelectbox div[data-baseweb="select"] { background: var(--panel) !important; border: 1px solid var(--dim) !important; border-radius: 0 !important; }
...
.stButton > button { background: transparent; color: var(--cream); border: 1px solid var(--dim);
  ...
  font-size: 0.7rem; letter-spacing: 2px; text-transform: uppercase; width: 100%; transition: all 0.18s; }
...
.stLinkButton a { border: 1px solid var(--dim) !important; border-radius: 0 !important;
  ...
  letter-spacing: 1px !important; color: var(--muted) !important; text-transform: uppercase !important; }
...
.stTable th { background: var(--bg) !important; color: var(--red) !important;
  ...
  letter-spacing: 3px !important; text-transform: uppercase !important; border-bottom: 1px solid var(--dim) !important; }
.stTable td { color: var(--cream) !important; background: var(--panel) !important;
  font-family: 'JetBrains Mono', monospace !important; font-size: 0.75rem !important; border-bottom: 1px solid var(--border) !important; }
.stAlert { background: var(--panel) !important; border: 1px solid var(--dim) !important;
  border-left: 2px solid var(--red) !important; color: var(--cream) !important; border-radius: 0 !important; }
...
def seed_database_if_empty():
    """Load data.csv into news_signals if table is empty."""
    try:
        ...
        if count == 0:
            # Try to load from data.csv
            try:
                df = pd.read_csv("data.csv")
                # Normalize column names to lowercase
                df.columns = [c.lower().strip() for c in df.columns]

                # Map CSV columns to our schema: actor_name, sentiment_score, source_url, location_name
                col_map = {}
                for col in df.columns:
                    if "actor" in col or "name" in col and "location" not in col:
                        col_map["actor_name"] = col
                    elif "sentiment" in col or "tone" in col or "score" in col:
                        col_map["sentiment_score"] = col
                    elif "url" in col or "source" in col:
                        col_map["source_url"] = col
                    elif "location" in col or "country" in col or "geo" in col or "action" in col:
                        col_map["location_name"] = col

                # Build insert rows
                rows = []
                for _, row in df.iterrows():
                    actor = str(row.get(col_map.get("actor_name", df.columns[0]), "Unknown"))[:255]
                    try:
                        score = float(row.get(col_map.get("sentiment_score", ""), 0) or 0)
                    except:
                        score = 0.0
                    url = str(row.get(col_map.get("source_url", ""), ""))[:2000]
                    location = str(row.get(col_map.get("location_name", ""), "Unknown"))[:255]
                    rows.append((actor, score, url, location))

                psycopg2.extras.execute_values(
                    cur,
                    "INSERT INTO news_signals (actor_name, sentiment_score, source_url, location_name) VALUES %s",
                    rows,
                    page_size=500
                )
                conn.commit()
                result = f"Seeded {len(rows)} records."
            except Exception as e:
                conn.rollback()
                result = f"CSV seed failed: {e}"
        else:
            result = f"DB has {count} records."
        conn.close()
        return result
    except Exception as e:
        return f"DB connection failed: {e}"

seed_status = seed_database_if_empty()
...
def load_countries():
    try:
        conn = get_db_connection()
        raw = pd.read_sql("SELECT DISTINCT location_name FROM news_signals WHERE location_name IS NOT NULL LIMIT 500", conn)["location_name"].tolist()
        conn.close()
        cleaned = sorted(set([str(l).split(",")[-1].strip() for l in raw if l and len(str(l)) > 2]))
        return cleaned if cleaned else ["India", "USA", "Russia", "UK", "China", "Germany", "France", "Brazil"]
    except:
...
with st.sidebar:
    ...
    <div style='text-align:center; padding:1rem 0 1.5rem;'>
      <div style='font-family:"Playfair Display",serif; font-size:1.4rem; color:#e0e0d1; letter-spacing:6px; text-transform:uppercase;'>
    ...
    def render_slot(text, spinning=False):
        ...
        <div style='background:#0d0d0d; border:1px solid #1e1e1e; border-left:2px solid #a01a1a;
          ...
          font-size:1.2rem; color:{color}; letter-spacing:4px; text-transform:uppercase; margin-bottom:10px;'>
          {str(text).upper()[:20]}
        </div>""", unsafe_allow_html=True)
    ...
    # Audit section — runs a simple SQL insert instead of a missing stored procedure
    st.markdown("<div style='height:1px; background:#1e1e1e; margin:1.5rem 0;'></div>", unsafe_allow_html=True)
    ...
    if st.button("Run Bias Audit"):
        try:
            ...
            # Log the audit event directly — no stored procedure needed
            cur.execute(
                "INSERT INTO bias_reports (report_reason) VALUES (%s)",
                (f"System audit triggered for target: {st.session_state.target}",)
            )
    ...
    # Show DB seed status in expander
    with st.expander("DB Status", expanded=False):
        st.caption(seed_status)
...
<div style='border-bottom:1px solid #1e1e1e; padding-bottom:1rem; margin-bottom:1.5rem;
  ...
  <div>
    <div style='font-family:"Playfair Display",serif; font-size:2rem; color:#e0e0d1; letter-spacing:8px; text-transform:uppercase;'>
      BIAS<span style='color:#a01a1a;'>SENTINEL</span>
    </div>
    <div style='font-size:0.55rem; color:#333; letter-spacing:4px; text-transform:uppercase; margin-top:4px;'>
      Global Media Intelligence Engine &nbsp;·&nbsp; Neon Cloud &nbsp;·&nbsp; PostgreSQL
    </div>
  </div>
...
def fetch_signals(geo):
    try:
        conn = get_db_connection()
        df = pd.read_sql(
            "SELECT * FROM news_signals WHERE location_name ILIKE %s LIMIT 1000",
            conn, params=(f"%{geo}%",)
        )
        conn.close()
        return df
...
if st.session_state.tab == "dashboard":
    if filtered_df.empty:
        ...
        <div style='background:#111; border-left:2px solid #a01a1a; padding:2rem;
          ...
          No signal matches for this region — try spinning the roulette or selecting another target.
        </div>""", unsafe_allow_html=True)
        st.stop()
    ...
    most_extreme = df_grouped.iloc[0]["CHANNEL"][:14] if not df_grouped.empty else "N/A"
    stat_card(s4, "Most Extreme", most_extreme)
    ...
    for _, row in df_grouped.iterrows():
        ...
        badge = ("background:#a01a1a18; color:#a01a1a; border:1px solid #a01a1a33;"
                 if "NEGATIVE" in row["ANALYSIS"] else
                 ("background:#00A86B18; color:#00A86B; border:1px solid #00A86B33;"
                  if "POSITIVE" in row["ANALYSIS"] else
    ...
    with ch2:
        ...
        fig2.update_layout(**PLOT, title=None,
    ...
    for i, (_, row) in enumerate(sample.iterrows()):
        ...
        with cols[i % 3]:
            st.markdown(f"""
            <div style='background:#111; border-top:2px solid {accent}; padding:12px 14px; margin-bottom:8px;'>
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    for col, country, avg, color in [(m1, c_a, avg_a, "#a01a1a"), (m2, c_b, avg_b, "#e0e0d1")]:
        col.markdown(f"""
        <div style='background:#111; border-top:2px solid {color}; padding:2rem; text-align:center; margin-bottom:1rem;'>
    ...
    fig = go.Figure(data=[
        go.Bar(name=c_a.upper(), x=[c_a.upper()], y=[avg_a], marker_color="#a01a1a", marker_line_width=0),
        go.Bar(name=c_b.upper(), x=[c_b.upper()], y=[avg_b], marker_color="#444", marker_line_width=0),
    ])
...
<div style='position:fixed; bottom:0; left:0; width:100%; background:#0d0d0d;
  ...
  <span style='font-size:0.55rem; color:#2a2a2a; letter-spacing:2px; text-transform:uppercase;'>Neon Cloud &nbsp;|&nbsp; PostgreSQL</span>
</div>
""", unsafe_allow_html=True)
